...
const emailUrl =
  "mailto:info@downtown4democracy.org?subject=Gun%20Regulation%20Map";
      ...
      <ul>
        <li>
          <Link className="active" to="/">
            <strong>D4D</strong> Gun Regulation
          </Link>
        </li>
        <li>
...
const About = () => {
  return (
    <div className="pageContainer">
      <h5>About</h5>
      <p>
        <strong>D4D Gun Regulation</strong> is a tool created by Downtown for
        Democracy that displays the organization's ranks given to political
        candidates for their prospective views toward stricter gun regulation
        based on voting records and public statements.
      </p>
    </div>
  );
};
...
const Contact = () => {
  return (
    <div className="pageContainer">
      <h5>Contact</h5>
      <a href={emailUrl} target="_blank" className="site-email">
        {emailAddress}
      </a>
    </div>
  );
...
const UsMap = () => {
  return (
    <div id="Map" className="pageContainer">
      <h5>Choose Your State:</h5>
      <svg
  ...
  return (
    <div id="State">
      ...
      <div className="stateBodyContainer">
        <NavAndPanesAndVotingCard stateId={id} />
      </div>
    </div>
  );
};
...
class NavAndPanesAndVotingCard extends React.Component {
  ...
  constructor(props) {
    ...
    this.handleRadioButtonPush = this.handleRadioButtonPush.bind(this);
  }
  handleChange(event) {
    ...
    if (event.target.checked) {
      ...
      value.push([event.target.id.split(",")]);
      console.log(this.state.value);
    } else {
      // or remove the value from the unchecked checkbox from the array
      index = value.indexOf([event.target.id.split(",")]);
    ...
    handleRadioButtonPush(rSelected) {
      const value = this.state.value;
      let index;

      // add the x.name of the radio button to options array
      value.push(rSelected);
      console.log(this.state.value);
            ...
            <div>
              <ButtonGroup>
                <Candidates
                  ...
                  handleRadioButtonPush={this.handleRadioButtonPush}
                />
              </ButtonGroup>
            </div>
          </div>
...
class Candidates extends React.Component {
  constructor(props) {
    // props: candidates, legislature, district, handleChange, handleRadioButtonPush
    super(props);
    this.state = {
      rSelected: []
    }
  ...
  onRadioBtnClick(rSelected) {
    ...
    rSelected.
    this.props.handleRadioButtonPush(this.state.rSelected)
  }
  ...
  render() {
    const candidates = this.props.candidates.map(x => (
      <Button key={x.name} sm="6" className="candidate"
        id={
          this.props.legislature +
          "," +
          this.props.district +
          "," +
          x.name +
         ...
         onClick={() => this.onRadioBtnClick(this.props.legislature +
                "," +
                this.props.district +
                "," +
                x.name +
                "," +
                x.img)} 
         active={this.state.rSelected === x.name}
       >
        <img className="portrait" src={x.img} alt={x.name} />
        <br />
        <div id="textbox">
          <p className="alignleft">{x.name}</p>
          <p
            style={
              x.grade === "A" || x.grade === "B"
                ? goodGradeStyle
            ...
            className="alignright letterGrades"
          >
            {x.grade}
          </p>
...
class PrintDialogue extends React.Component {
  ...
  render() {
    return (
      <div id="PrintDialogue">
        {this.props.value.length === 0 && (
          <div style={{ color: "grey", fontSize: ".8rem" }}>
            <em>
              Click the checkbox by a candidate's photo to save them to your
              vote card.
            </em>
          </div>
        )}
...
class MyVotes extends React.Component {
  ...
  render() {
    ...
    // const myVotesSections = this.props.legislatures.map(x =>
    //   // x = stateInfo.{stateId}.legislatures[i]
    //   <div key={x.id} tabId={x.id}>
    //     {x.districts.map(y =>
    //       // x = stateInfo.{stateId}.legislatures[i]districts[i]
    //       <div>
    //         {
